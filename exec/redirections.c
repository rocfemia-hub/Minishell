...
/*   Updated: 2025/09/09 15:21:41 by roo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void set_redirections(t_com *list)
{
	...
	if (list->redirects == NULL)
		return;
	while(list->redirects->input_file[i])
		i++;
	while(list->redirects->output_file[j])
		j++;
	while(list->redirects->input_file[k])
		k++;
    if (list->redirects->output_file[j]) // Redirección de salida > "¿Hay un archivo para escribir?"
    {
    ...
    if (list->redirects->append_file[k]) // Append >> "¿Hay archivo para añadir?"
    {
