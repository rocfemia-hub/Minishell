...
char **copy_redirect_matrix(char **args, int start, int end) //copiar matriz de args sin el simbolo de redireccion y el nombre del archivo
{
    int i;
    int j;
    char **new_arg;
...
void fill(t_com *commands, int start, int end, char *redirect) // dejar solo arg en args y rellenar flags
{
    char **new_arg;
...
void parser_redirects(t_com *commands, char *redirect) // chequear si despues de la redireccion hay archivo o antes 
{
...
void find(t_com *commands) // look for < or >
{
