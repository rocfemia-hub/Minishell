...
void			aux_redirects(t_com *commands, char *redirect_pos,
					int type, char *redirect);
...
void			process_quote_char(char current_char, char *new_arg,
					int *k, char *quote);
...
void			aux_clean_redirects_cmd(t_com *commands,
...
// UTILS_EXPANDER.C
char			*extract_and_expand_var(char *line, int vstart,
					int *vlen, t_vars *vars);
char			*expand_var_in_quotes_args(char *line, int *k,
					char *token, t_vars *vars);
char			*handle_dollar_in_quotes(char *line, int *k,
					int *start, t_vars *vars);
char			*process_inside_double_quotes(char *line,
					int start, int end, t_vars *vars);
...
void			handle_redirect_array(char ***arr, int *flag,
					char *file, t_com *commands);
